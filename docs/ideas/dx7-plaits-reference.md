...
Reference notes on the Yamaha DX7's FM architecture and how Émilie Gillet implemented it in Mutable Instruments Plaits (`../eurorack/plaits/dsp/fm/`). Not a spec to implement — a reference for understanding the full picture of multi-operator FM.

## DX7 overview

The DX7 is a 6-operator FM synth with 32 algorithms. Each operator is a sine oscillator with its own 4-stage envelope, level, frequency ratio, detune, and keyboard scaling. The algorithm determines how operators are wired together — which ones modulate which, and which ones output audio.
...
The phase is a 32-bit integer where the full range (0 to 2^32) represents one cycle. Overflow wraps naturally — no modulo needed. This is more efficient than the float-based 0-1 phase that DaisySP uses, and gives better phase precision.
...
Every algorithm has exactly one operator that can self-modulate. This is a DX7 constraint — cross-feedback (operator A feeds back into operator B which feeds back into A) would create a circular dependency that's hard to render in a single pass.
...
Plaits encodes each algorithm as 6 bytes — one opcode per operator. Each opcode packs:
...
**Wide/additive** (algorithms 19-32): Many operators output directly with less modulation. Approaches additive synthesis at low FM depths. Some have modulation sharing — one modulator feeds multiple carriers.
...
The DX7 envelope is a 4-stage envelope, but it's not a standard ADSR. Each stage has a rate and a target level — the envelope moves from whatever level it's at toward the target at the specified rate. The stages are:
...
This is more flexible than ADSR because any stage can go up or down — the sustain level doesn't have to be below the decay level. You can create envelopes that rise in multiple stages, dip and recover, or have non-zero release targets.
...
**Ratio mode** (default): Frequency = note frequency × coarse ratio × fine adjustment. Coarse ratios are looked up from a table (0.5, 1, 2, 3, ... 31). Fine adds sub-ratio detuning. The operator tracks the keyboard — play a higher note, the operator runs faster.
...
The DX7/Plaits architecture is the ceiling — 6 operators, 32 algorithms, full SysEx compatibility. We don't need any of that for a game sound engine. But the concepts are directly applicable:
...
- **Feedback** is a high-value addition — one float parameter and a 2-sample buffer per operator, but it adds significant harmonic range. Worth implementing.
