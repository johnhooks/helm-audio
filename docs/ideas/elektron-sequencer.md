...
Reference notes on the Elektron sequencer as implemented across their instruments (Digitakt, Digitone, Analog Four, Syntakt, Octatrack). Not a spec to replicate — a reference for understanding what makes their approach to step sequencing distinctive, and how it informs what we're building.

## Core architecture

Elektron sequencers are 16-step grids displayed on 16 physical trig buttons. Patterns can be 16, 32, 48, or 64 steps long, accessed through pages. A 64-step pattern is four pages of 16. This keeps the interface tactile — always 16 buttons — while supporting longer sequences.
...
Two recording modes: **grid recording** (step-entry, press buttons to place/edit trigs) and **live recording** (play in real time while the pattern loops, captures note, velocity, and micro-timing). Both produce the same data — they're just different input methods for the same grid.
...
**Trigless locks** place parameter lock data on a step without retriggering the sound. This is essential for modulating parameters on a sustained note — change filter cutoff midway through a held pad without restarting the amplitude envelope. The sound continues, only the parameters change.
...
2. On any step with a trig, you hold the trig button and turn a knob — that parameter is "locked" to that step.
3. When the sequencer reaches that step, it sends the locked value instead of the base value.
4. After the trig's note duration ends, the parameters revert to the base patch. No interpolation — values snap to the locked value on the step, then snap back.

Every parameter on every parameter page is lockable: filter cutoff, resonance, envelope times, LFO speed, oscillator tuning, effects sends, sample start point — all of it.

**Sound locks** are a special form of p-lock that swap the entire sound/preset on a per-step basis. Hold a trig, browse the sound pool, and assign a different sound to that step. This means a single track can play a kick on step 1, a snare on step 5, and a clap on step 9 — effectively multiplying the number of available voices beyond the track count.
...
Each trig can be nudged forward or backward from its grid position with sub-step resolution. In grid recording, you hold a trig and press left/right to access the micro-timing popup. Each step stores its own offset independently.

The resolution is fine enough for humanized grooves, flamming, or ghost notes that sit just behind the beat. Scale multiplier settings (2x, 3/4x) interact with micro-timing resolution since they change the effective step duration.
...
**NEI / !NEI**: Fires if the most recently evaluated trig condition on the neighboring track (immediately before) was true/false. Creates inter-track dependencies — one track's behavior influences another's.

Trig conditions can be combined with parameter locks (conditional locks) — a parameter change that only happens under certain conditions.

## Retrig

Retrig subdivides a single step into multiple rapid retriggerings. Three parameters per step:

- **Rate**: subdivision from 1/1 to 1/80. At 1/2, you get 2 retrigs per step. At 1/64, rapid-fire buzz rolls.
...
Swing delays every second sixteenth note (even-numbered steps). Default is 50% (no swing), maximum around 80%. Stored per-pattern. On some devices (Machinedrum, Octatrack), you can toggle which specific steps are affected by swing, enabling non-standard groove patterns.
...
**Trigs as typed actions.** Our `Trigger` variant (`NoteOn`, `NoteOff`, `FadeOut`) maps directly to Elektron's trig types. A step isn't "play a note" — it's a bundle of actions that includes patch loading, triggering, and parameter overrides.
...
**Sound locks.** Our `patchIndex` field on a step loads an entirely different voice configuration before triggering. Combined with a trig on the same step, this achieves the same effect as Elektron's sound locks — one track, multiple timbres.
...
**Micro-timing.** Our `int8_t microTiming` offset per step (-5 to +5 ticks within a 6-tick window) nudges trigs earlier or later. The cursor + peek model handles negative offsets by looking ahead to the next step — same concept as Elektron, different implementation.
...
**Fill mode.** A global boolean toggled by the JS layer. Steps with a Fill condition fire only when Fill is active. Useful for responsive game audio — game state toggles Fill, and pre-authored fills play automatically.
...
**Trigless locks.** Parameter changes without retriggering. Currently our steps either have a trig or don't — a step with only param locks and no trig would achieve this. Might already work if we dispatch locks regardless of trig state; needs verification.
...
**Complex arpeggiator.** Elektron's per-track arpeggiator is deeply integrated with their keyboard workflow. Our patterns are generated, not played — if we want arpeggiation, JS generates the pattern.
...
Elektron's innovation isn't any single feature — it's the interaction between features in a unified system. Parameter locks turn a step sequencer into a modulation sequencer. Trig conditions turn a fixed loop into an evolving phrase. Sound locks decouple tracks from voices. Per-track length creates polymetric complexity. Trigless locks separate parameter automation from note triggering.

We've implemented the structural foundation — typed trigs, param locks, sound locks, polymetric tracks, micro-timing. The next layer of expressiveness comes from the conditional system (A:B counts, probability, fill, PRE/NEI), which builds on what's already there without changing the core architecture. The sequencer already tracks loop count per track; trig conditions are a dispatch-time filter on existing data.
